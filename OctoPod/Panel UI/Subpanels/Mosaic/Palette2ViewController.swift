            ...
            if let command = data["command"] as? String {
                if command == "p2Connection", let connected = data["data"] as? Bool {
                    ...
                    if pings.isEmpty {
                        ...
                        if let messages = Palette2Utils.pingPongMessage(history: pings, index: pings.count - 1, reversed: false) {
                            DispatchQueue.main.async {
                    ...
                    if pongs.isEmpty {
                        ...
                        if let messages = Palette2Utils.pingPongMessage(history: pongs, index: pongs.count - 1, reversed: false) {
                            DispatchQueue.main.async {
